      ...
      <div className="min-h-screen bg-gradient-to-b from-background to-muted overflow-x-hidden">
        <header className="border-b border-border bg-card/50 backdrop-blur sticky top-0 z-10">
          <div className="flex h-14 md:h-16 items-center justify-between px-4 md:px-6">
            <Button variant="ghost" size="sm" onClick={() => navigate("/dashboard")} className="-ml-2 md:ml-0">
              <ArrowLeft className="h-4 w-4 mr-2" /> 
              <span className="hidden sm:inline">Dashboard</span>
              <span className="inline sm:hidden">Back</span>
            </Button>
            <h1 className="font-bold text-base md:text-lg">Quiz Results</h1>
          </div>
        </header>
        ...
        <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-4 md:space-y-6 pb-8">
          <Card className="bg-gradient-to-r from-primary/10 to-accent/10 border-none shadow-xl text-center">
            <CardHeader className="pb-2 md:pb-6">
              {score >= questions.length / 2 ? <Trophy className="h-10 w-10 md:h-12 md:w-12 mx-auto text-primary" /> : <Brain className="h-10 w-10 md:h-12 md:w-12 mx-auto text-muted-foreground" />}
              <CardTitle className="text-2xl md:text-3xl mt-2">Quiz Complete!</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-base md:text-lg mb-4">You scored <span className="font-bold text-primary">{score}</span> out of {questions.length}</p>
          ...
          <Card>
            <CardHeader className="p-4 md:p-6"><CardTitle className="text-lg md:text-xl">Review Your Answers</CardTitle></CardHeader>
            <CardContent className="space-y-4 p-4 md:p-6 pt-0 md:pt-0">
              {questions.map((q, i) => (
                <div key={q.id} className="p-3 md:p-4 rounded-lg border space-y-2">
                  <div className="flex justify-between items-start gap-3">
                    <h4 className="text-sm md:text-base font-medium leading-snug flex-1">{i + 1}. {q.question}</h4>
                    {selectedAnswers[i] === q.correctAnswer ? <CheckCircle className="h-5 w-5 text-success flex-shrink-0 mt-0.5" /> : <XCircle className="h-5 w-5 text-destructive flex-shrink-0 mt-0.5" />}
                  </div>
                  {q.options.map((opt, idx) => (
                    <div key={idx} className={`p-2 rounded text-xs md:text-sm ${idx === q.correctAnswer ? "bg-success/20 text-success" : selectedAnswers[i] === idx ? "bg-destructive/20 text-destructive" : "bg-muted"}`}>{opt}</div>
                  ))}
                  <p className="text-xs text-muted-foreground mt-2 leading-relaxed"><b>Explanation:</b> {q.explanation}</p>
                </div>
              ))}
            </CardContent>
          </Card>
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur sticky top-0 z-10">
        <div className="flex h-14 md:h-16 items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <Badge variant="secondary" className="h-7 md:h-auto"><Clock className="h-3 w-3 mr-1" /> {formatTime(timeElapsed)}</Badge>
            <Badge variant="outline" className="h-7 md:h-auto">{currentQuestion + 1} / {questions.length}</Badge>
          </div>
        </div>
      </header>

      <div className="p-4 md:p-6 max-w-3xl mx-auto flex flex-col h-[calc(100vh-64px)] md:h-auto">
        ...
        <Card className="shadow-lg border-none flex-1 flex flex-col">
          <CardHeader className="p-4 md:p-6 pb-2">
            <CardTitle className="text-lg md:text-2xl leading-tight">{questions[currentQuestion].question}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 p-4 md:p-6 pt-2 overflow-y-auto">
            {questions[currentQuestion].options.map((opt, idx) => (
              <motion.div key={idx} whileHover={{ scale: 1.01 }} transition={{ duration: 0.2 }}>
                <Button
                  variant={selectedAnswers[currentQuestion] === idx ? "default" : "outline"}
                  className={`w-full justify-start text-left p-3 md:p-4 h-auto whitespace-normal transition-colors duration-200
                ...
                >
                  <span className="mr-3 font-mono text-sm md:text-base opacity-70 flex-shrink-0">{String.fromCharCode(65 + idx)}.</span>
        ...
        <div className="flex justify-between mt-4 md:mt-6 pb-4 md:pb-0">
          <Button onClick={previousQuestion} disabled={currentQuestion === 0} variant="outline" className="h-10 md:h-11">
            <ArrowLeft className="h-4 w-4 mr-2" /> Previous
          </Button>
          <Button
            onClick={nextQuestion}
            disabled={selectedAnswers[currentQuestion] === undefined}
            className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white hover:opacity-90 h-10 md:h-11"
          >
